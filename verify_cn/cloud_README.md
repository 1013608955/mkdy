> **⚠️ 现状更新（2026-08-24）**：容器已从「常开自跑」切换为「CI 按需唤醒」模式——
> GitHub Actions `hds-cycle.yml` 每小时自动 唤醒→验证→关机，核时消耗约 120-150/月
> （常开为 1440/月）。本文其余内容（初始化引导 / .env 配置）仍适用；
> 当前运行架构以仓库根 [README.md](../README.md) 为准。
>
# 容器版（华为云开发环境）部署指南
