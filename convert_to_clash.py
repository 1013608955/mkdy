...
def decode_base64_file(file_path):
    """解码Base64文件内容（修复b64decode参数错误，增加容错）"""
    try:
        ...
        # 修复：先Base64解码（无errors参数），再字符串解码（处理错误）
...
def parse_ss(encoded_part, server, port):
    ...
    try:
        ...
        # 修复：Base64解码移除errors参数
        byte_data = base64.b64decode(encoded_part)
        decoded = byte_data.decode('utf-8', errors='ignore')
    ...
    for proto, pattern in PROTOCOL_PATTERNS.items():
        ...
        if match:
            if proto == 'vmess':
                ...
                # 修复：函数名拼写一致，且parse_general已提前定义
